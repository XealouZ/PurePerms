...
use _64FF00\PureChat\Clans\ClansProNew;
use _64FF00\PureChat\Clans\ClansProOld;
use _64FF00\PureChat\Clans\FahmiCyber_Clans;
...
class PureChat extends PluginBase
{
    ...
    */
    ...
    private function fixOldConfig()
    {
        ...
        if(!isset($tempData["default-clans-plugin"]))
            $tempData["default-clans-plugin"] = null;
        ...
        if(isset($tempData["custom-no-clan-message"]))
            unset($tempData["custom-no-clan-message"]);
    ...
    private function fixOldData($string)
    {
        ...
        $string = str_replace("{clan}", "{clan_rank}{clan_name}", $string);
    ...
    private function loadClansPlugin()
    {
        $clansPluginName = $this->config->get("default-clans-plugin");

        if($clansPluginName === null)
        {
            $this->getLogger()->notice("No valid clans plugin in default-clans-plugin node was found. Disabling clans plugin support.");
        }
        else
        {
            switch(strtolower($clansPluginName))
            {
                case "clanspro":

                    $clansPro = $this->getServer()->getPluginManager()->getPlugin("ClansPro");

                    if($clansPro !== null)
                    {
                        if(version_compare($clansPro->getDescription()->getVersion(), "1.5b1") === -1)
                        {
                            $this->clansAPI = new ClansProOld();

                            $this->getLogger()->notice("ClansPro-OLD support enabled.");

                            break;
                        }
                        else
                        {
                            $this->clansAPI = new clansProNew();

                            $this->getLogger()->notice("ClansPro-NEW support enabled.");
                ...
                case "fahmicyber-clans":

                    if($this->getServer()->getPluginManager()->getPlugin("FahmiCyber-Clans") !== null)
                    {
                        $this->clansAPI = new FahmiCyber_Clans();

                        $this->getLogger()->notice("FahmiCyber-Clans support enabled.");
    ...
    */
    ...
    public function applyPCTags($string, Player $player, $message, $levelName)
    {
        ...
        if($this->clansAPI !== null)
        {
            $string = str_replace("{clan_name}", $this->clansAPI->getPlayerClan($player), $string);
            $string = str_replace("{clan_rank}", $this->clansAPI->getPlayerRank($player), $string);
        }
        else
        {
